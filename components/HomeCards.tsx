      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 mb-24">
          {cards.map((card, index) => (
            <Link key={index} href={card.link} className="block group relative overflow-hidden
              rounded-2xl border-2 border-black bg-white p-6
              transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">

              {/* Water-fill hover layer */}
              <div className="absolute inset-0 bg-gradient-to-br from-[#C08497] to-[#9D174D]
              ...
              <div className="relative z-10">
                ...
                <p className="text-slate-700 group-hover:text-slate-200
        ...
        <div className="relative rounded-3xl bg-gradient-to-br from-[#5B0F1B] to-[#8B1E2D]
          px-10 py-14 text-white overflow-hidden shadow-2xl">
          <div className="absolute -top-20 -right-20 w-80 h-80 bg-white/10 rounded-full blur-3xl" />
          <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-black/10 rounded-full blur-3xl" />

          {/* Purple accent orb — ties into the framework page */}
          <div className="absolute top-8 right-32 w-32 h-32 rounded-full blur-2xl"
          ...
          <div className="relative max-w-4xl">
            ...
            {/* FIXED: now links to /framework */}
            <div className="mt-8">
              <Link href="/framework"
                className="group relative inline-block overflow-hidden rounded-full
                  border border-white/50 px-6 py-3 cursor-pointer">
                <div className="absolute inset-0 bg-white translate-y-full
                  group-hover:translate-y-0 transition-transform duration-700 ease-in-out" />
                <span className="relative z-10 font-medium text-white
                  group-hover:text-[#7A1C28] transition-colors duration-300">
                  Explore framework →
                </span>
              </Link>
            </div>
